 ...
 * 진료받을 병원.
 *
 * **주소는 병원 찾기(1m-B)에서 고른 경우에만 있다.** 한동안 서버가 이름만 줘서 비어
 * 있었는데(#155) 우리가 요청해 자리가 생겼다(#187). 홈·목록에서 누른 줄을 타고 들어온
 * 카드는 여전히 이름뿐이다 — `CardSummary`에 주소가 없다(Backend#101). 값이 없으면 줄을
 * 그리지 않는다.
 */
