...
| 항목 | 상태 |
| -- | -- |
| 유닛 테스트 | `IntakeViewModel` 33건, `VisitRecordViewModel` 23건, `SessionViewModel` 23건, `BriefCardViewModel` 27건, `ScheduleAddViewModel` 20건, `BodyMapGeometry` 17건, `HealthEditViewModel` 17건, `RecordViewModel` 17건, `CalendarDayViewModel` 20건, `CalendarViewModel` 15건, `BriefCardListViewModel` 14건, `HomeRepository` 14건, `HospitalPickViewModel` 14건, `ProfileSetupViewModel` 14건, `LoginViewModel` 13건, `DesignToken` 12건, `RecordDetailFixtures` 12건, `MyProfileViewModel` 12건, `RecordDetailViewModel` 18건, `BodyMapSearchState` 11건, `CardMapping` 14건, `VectorParity` 11건, `BodyMapLayout` 10건, `IntakeSessionActions` 10건, `AppointmentRepository` 8건, `BodyPartSearch` 8건, `HomeViewModel` 7건, `MedicalMateSeverity` 7건, `TokenAuthenticator` 7건, `VisitNoteViewModel` 13건, `HomeSchedule` 4건, `PretendardFont` 4건, `ProgressIndicator` 4건, `SegmentedControl` 3건, 템플릿 1개 |
...
| 디자인 트랙 확인 대기 | 아홉 건이다. (1) 인체도 화면의 프레임이 시안에 없는 이유 (2) `Card`의 최소 높이 116과 시안 1j-1-D의 줄 높이 80 중 어느 쪽 (3) 선택 건수를 알리는 자리 — 1j-1은 하단 버튼 글자, 1j-4는 `Select Bar`까지 (4) 기록 상세 타임라인 차례 — `1j-3`은 오래된 것부터, `1j-3-R`은 최신부터인데 코드는 최신부터 (5) `1r-4` 상단 왼쪽이 ×인지 ‹인지 — 네 프레임 중 셋이 ×라 ×를 따랐다 (6) `1r-2-C`와 `1r-1-S` 중 어느 쪽 (7) 1b의 "잘 모르겠어요 · 없어요" 보조 버튼. 우리가 뺐는데(#67) 서버가 `NONE`과 `UNKNOWN`을 다른 값으로 받아서 다시 봐야 한다. 다만 그 버튼은 한 개에 두 뜻이 묶여 있어 그대로 넣어도 갈리지 않는다 (8) `Hospital Card`와 1m 결과 줄의 주소 자리 — 서버가 이름만 줘서 채울 값이 없다. 지금은 값이 없으면 줄을 그리지 않는다 (9) 1m이 검색 전에 빈 화면으로 열린다 — 시안은 후보를 먼저 보여주는데 서버 검색은 질의가 있어야 답이 온다. 목록은 #119·#121·#150·#155에 있다 |
