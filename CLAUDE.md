...
| 디자인 트랙 확인 대기 | 열한 건을 #189로 모아 올렸다. 앞의 셋은 지금 막히는 것이다 — (1) 1q-1 편집에서 줄을 더하는 자리와 더한 줄의 이름(#184) (2) 하루에 진료가 둘일 때 일자 화면(#179) (3) 저장한 기록을 고칠 진입. `PATCH /api/visits/{visitId}`가 열렸는데 1j-3에 편집 자리가 없다. 뒤의 여덟은 대조 때부터 남아 있던 것이고 코드는 괄호 안대로 둔다 — 인체도 프레임이 시안에 없는 이유, `Card` 최소 높이 116과 1j-1-D의 80, 기록 상세 타임라인 차례, `1r-4` 상단 왼쪽 ×·‹, `1r-2-C`와 `1r-1-S`, 1b의 "잘 모르겠어요 · 없어요" 보조 버튼, 브리핑 카드의 병원 블록에 주소를 실을지, 1m이 검색 전에 빈 화면으로 여는 것 |
