...
| 항목 | 상태 |
| -- | -- |
| 유닛 테스트 | `IntakeViewModel` 33건, `VisitRecordViewModel` 23건, `SessionViewModel` 23건, `BriefCardViewModel` 27건, `ScheduleAddViewModel` 21건, `BodyMapGeometry` 17건, `HealthEditViewModel` 17건, `RecordViewModel` 17건, `CalendarDayViewModel` 20건, `CalendarViewModel` 15건, `BriefCardListViewModel` 14건, `HomeRepository` 14건, `HospitalPickViewModel` 14건, `ProfileSetupViewModel` 14건, `LoginViewModel` 13건, `DesignToken` 12건, `RecordDetailFixtures` 12건, `MyProfileViewModel` 12건, `RecordDetailViewModel` 18건, `BodyMapSearchState` 11건, `CardMapping` 14건, `VectorParity` 11건, `BodyMapLayout` 10건, `IntakeSessionActions` 10건, `SessionRepository` 4건, `AppointmentRepository` 8건, `BodyPartSearch` 8건, `HomeViewModel` 7건, `MedicalMateSeverity` 7건, `TokenAuthenticator` 7건, `VisitNoteViewModel` 13건, `HomeSchedule` 4건, `PretendardFont` 4건, `ProgressIndicator` 4건, `SegmentedControl` 3건, 템플릿 1개 |
...
| 증상 정리(1l·1c·1d·1i) | 네 단계 화면 구현. AI 응답은 연동됐고 **음성 인식만 미연동**이다 — 패널과 상태까지 있고 받아쓰기가 없다. 4단계(1i)의 목록은 `SessionResponse.questionCandidates`를 `rank` 차례로 채운다(#173). 환자가 한 번이라도 손댔으면 후보로 덮지 않는다 — 덮으면 지운 질문이 되살아난다. **상단 바 제목이 "기록"이고 진행 라벨이 "증상 정리"다** — 1c-2·1c-4·1c-5·1i 네 장이 그렇게 그려져 있다 |
